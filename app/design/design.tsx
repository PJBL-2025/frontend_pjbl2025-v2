...
export default function Design() {
  ...
  // nanti disini lek kirim datanya
};
            ...
            <View className="items-start w-full">
              ...
              <View className="flex-row justify-between items-start mt-4 w-full px-2 space-x-2">
                ...
                <View className="w-[48%] relative">
                  ...
                  {showDropdown && (
                    <View className="absolute top-[45px] left-0 right-0 bg-white rounded-xl shadow z-50 border border-gray-200">
                      {models.map((item) => (
                        <TouchableOpacity
                          key={item.value}
                          onPress={() => handleSelectModel(item.label, item.value)}
              ...
              <View className="mt-4 flex-row gap-2 justify-between w-full px-2">
                <TouchableOpacity
                  onPress={pickImage}
                  disabled={isLoading}
              ...
              <View className='mt-2 p-2 w-full gap-3'>
              <TextInput
                className="bg-white px-4 py-3 rounded-xl text-black"
                placeholder="Berikan catatan untuk penjual (Opsional)"
                value={textInput}
                onChangeText={setTextInput}
              />

              <TouchableOpacity
                className="bg-green-400 py-4 rounded-xl shadow items-center" onPress={handleCheckout}>
